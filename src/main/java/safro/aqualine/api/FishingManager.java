...
public class FishingManager {
    ...
    public static boolean handleRandomFish(ServerLevel level, Player player, FishingHook hook) {
    ...
    public static FishResult selectRandom(RandomSource random) {
        ...
        if (random.nextFloat() < entityWeight) {
            Optional<EntityFishResult> optional = selectRandomOf(ENTITIES, random);
            return optional.orElse(null);
        } else {
            Optional<ItemFishResult> optional = selectRandomOf(LOOT, random);
            return optional.orElse(null);
        }
    }

    private static <T extends FishResult> Optional<T> selectRandomOf(List<T> list, RandomSource random) {
